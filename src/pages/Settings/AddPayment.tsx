...
const AddPayment = ({ isOpen, onClose, paymentTerm, companyId }: AddPaymentProps) => {
    const dispatch = useDispatch();
    const { createPaymentTermError, isPaymentTermCreated, editPaymentTermError, isPaymentTermUpdated } = useSelector((state: any) => ({
    ...
    */
    const validator = useFormik({
        ...
        validationSchema: Yup.object({
            deposit: Yup.string().max(6, "Ensure that there are no more than 5 digits in Deposit.")
                .required('Deposit in % is required'),
            on_delivery: Yup.string().max(6, "Ensure that there are no more than 5 digits in Payment on Delivery.")
                .required('Payment on Delivery % is required'),
            receiving: Yup.string().max(6, "Ensure that there are no more than 5 digits in Receiving.")
                .required('Receiving in % is required'),
            payment_days: Yup.string().max(6, "Ensure that there are no more than 5 digits in Remaining in Payment Days.")
                .required('Remaining in Payment Days is required')
        }),
        onSubmit: values => {
            if (paymentTerm) {
        ...
        <Modal show={isOpen} onHide={onClose} size="lg">
            ...
            <Modal.Body className="p-0">
                <div className="px-5 pb-5">
                    <h1 className="mb-2 mt-0">{paymentTerm ? "Edit Payment Terms" : "Add Payment Terms"}</h1>
                    {
                    ...
                    <Form className="mt-3" noValidate onSubmit={validator.handleSubmit}>
                        <Form.Group className="mb-4">
                            <Form.Label htmlFor="usr">Deposit in %</Form.Label>
                        ...
                        <Form.Group className="mb-4">
                            <Form.Label htmlFor="usr">Payment on Delivery %</Form.Label>
                        ...
                        <Form.Group className="mb-4">
                            <Form.Label htmlFor="usr">Receiving in %</Form.Label>
                        ...
                        <Form.Group className="mb-4">
                            <Form.Label htmlFor="usr">Remaining in payment_days</Form.Label>
                        ...
                        <div>
                            <Button type="button" onClick={() => onCancel()} variant="outline-primary" className="mr-3" >cancel</Button>
                            <Button type="submit" variant="primary">{paymentTerm ? "Edit Payment Terms" : "Add Payment Terms"}</Button>
                        </div>
                    </Form>
